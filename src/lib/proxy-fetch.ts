 ...
 *
 * ⚠️ BASE_PATH 접두 필수 — 이 fetch는 브라우저 fetch라 Next.js 라우터를 거치지 않으므로
 *    basePath가 자동으로 붙지 않는다. 없으면 같은 도메인의 다른 서비스(api)로 오배송된다.
 *
...
export function proxyFetch(path: string, init?: RequestInit): Promise<Response> {
  ...
  return fetch(`${BASE_PATH}/api/proxy/${normalizedPath}`, init);
}
