...
import { triggerImportTemplateDownload } from './useImportTemplate';
import { ConsoleApiError } from '@/lib/api/console';

/**
 * 렌더 없이 Content-Type 가드 로직만 단위 테스트한다(vitest.config.ts가 jsdom 없이
 * node 환경만 제공 — 컴포넌트 렌더 테스트는 별도 인프라 필요, useStatsExcel.test.ts와 동일 인지).
 * 실패 경로(401 JSON, 비-xlsx CT)는 document API에 도달하기 전에 throw하므로 node 환경에서도
 * 검증 가능하다.
 */
describe('useImportTemplate — triggerImportTemplateDownload Content-Type 가드', () => {
  beforeEach(() => {
  ...
  it('401 + application/json 응답이면 xlsx로 저장하지 않고 ConsoleApiError를 던진다', async () => {
    ...
    await expect(triggerImportTemplateDownload('evt-1')).rejects.toBeInstanceOf(ConsoleApiError);
    await expect(triggerImportTemplateDownload('evt-1')).rejects.toMatchObject({
      status: 401,
      code: 'IMPORT_TEMPLATE_DOWNLOAD_FAILED',
    });
  });
