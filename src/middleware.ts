 ...
 *
 * ⚠️ **쿠키 존재 여부만 확인한다 — HMAC 검증도 역할 판정도 하지 않는다** (ADR-008).
 ...
 * `node:crypto`를 top-level import하므로 여기서 절대 import하지 않는다(ADR-018).
 */
export function middleware(request: NextRequest) {
  ...
  // 외부 좌표계로 나가는 리다이렉트 URL이라 BASE_PATH 수동 접두가 필수다(ADR-010) —
...
export const config = {
  // 보호 대상 5개 접두사만 나열하는 화이트리스트다(ADR-009). 부정 정규식은 게스트·키오스크
